...
namespace ZumasRevenge
{
	...
	{
		// Token: 0x06000E59 RID: 3673 RVA: 0x00092130 File Offset: 0x00090330
		public static int FPS_ADJUST(float fps)
		{
			return (int)(fps * 100f / 30f);
		}

		// Token: 0x06000E5A RID: 3674 RVA: 0x00092140 File Offset: 0x00090340
		...
		// Token: 0x06000E5B RID: 3675 RVA: 0x00092AFC File Offset: 0x00090CFC
		protected override void DrawShield(SexyGraphics g)
		{
			...
			for (int i = 0; i < Common._M(4); i++)
			{
				...
				{
					float num = this.mShieldAngle + (float)i * 3.14159f / 2f;
					float num2 = (float)Common._M(0);
					float num3 = (float)Common._M(0);
					if (!shieldQuadrantData.mDoExplodeAnim)
					{
						...
						cumulativeTransform.mTrans.RotateRad(num);
						cumulativeTransform.mTrans.Translate(Common._S(this.mX - num3), Common._S(this.mY - num2));
						composition.Draw(g, cumulativeTransform, -1, Common._DS(1f));
						if (g.Is3D())
						{
							g.PushState();
					...
					else
					{
						...
						cumulativeTransform2.mTrans.RotateRad(num);
						cumulativeTransform2.mTrans.Translate(Common._S(this.mX - num3), Common._S(this.mY - num2));
						if (!Common._eq(this.mAlphaOverride, 255f))
						{
					...
					if (shieldQuadrantData.mDoHitAnim)
					{
						...
						cumulativeTransform3.mTrans.RotateRad(num);
						cumulativeTransform3.mTrans.Translate(Common._S(this.mX - num3), Common._S(this.mY - num2));
						if (!Common._eq(this.mAlphaOverride, 255f))
						{
		...
		// Token: 0x06000E5C RID: 3676 RVA: 0x00092E69 File Offset: 0x00091069
		public override void DrawBelowBalls(SexyGraphics g)
		{
		}

		// Token: 0x06000E5D RID: 3677 RVA: 0x00092E6C File Offset: 0x0009106C
		protected override bool PreBulletUpdate(BossBullet b, int index)
		{
			...
			else if (Math.Abs(b.mState) == 2)
			{
				...
				if (b.mShotType == 1)
				{
					num = base.FireBulletAtPlayer(b, Common.FloatRange(base.mMinBulletSpeed, base.mMaxBulletSpeed), b.mX, b.mY);
					b.mTargetVX = b.mVX;
					b.mTargetVY = b.mVY;
				}
				else if (b.mShotType == 0)
				{
					b.mVY = Common.FloatRange(base.mMinBulletSpeed, base.mMaxBulletSpeed);
				}
		...
		// Token: 0x06000E5E RID: 3678 RVA: 0x00093130 File Offset: 0x00091330
		...
		// Token: 0x06000E5F RID: 3679 RVA: 0x0009316C File Offset: 0x0009136C
		...
		// Token: 0x06000E60 RID: 3680 RVA: 0x00093354 File Offset: 0x00091554
		protected override void BulletHitPlayer(BossBullet b)
		{
			if (b.mData != null)
			{
				...
				int num = Common._M(8);
				float num2 = Common.DegreesToRadians((float)Common._M(45));
				float num3 = Common.DegreesToRadians((float)Common._M(235));
				...
				for (int i = 0; i < num; i++)
				{
					...
					inkParticle2.mWidthPct = (inkParticle2.mHeightPct = Common.FloatRange(Common._M(0.25f), 0.75f));
					inkParticle2.mGravity = Common.FloatRange(Common._M(0.08f), Common._M1(0.13f));
					...
					inkParticle2.mJiggleRate = Common.FloatRange(Common._M(0.02f), Common._M1(0.03f));
				}
		...
		// Token: 0x06000E61 RID: 3681 RVA: 0x000935C4 File Offset: 0x000917C4
		...
		// Token: 0x06000E66 RID: 3686 RVA: 0x00093721 File Offset: 0x00091921
		public BossSquid() : this(null)
		{
		}

		// Token: 0x06000E67 RID: 3687 RVA: 0x0009372C File Offset: 0x0009192C
		...
		// Token: 0x06000E68 RID: 3688 RVA: 0x000937E0 File Offset: 0x000919E0
		...
		// Token: 0x06000E69 RID: 3689 RVA: 0x000938A0 File Offset: 0x00091AA0
		...
		// Token: 0x06000E6A RID: 3690 RVA: 0x000939A4 File Offset: 0x00091BA4
		...
		// Token: 0x06000E6B RID: 3691 RVA: 0x00093A44 File Offset: 0x00091C44
		...
		// Token: 0x06000E6C RID: 3692 RVA: 0x00093AE4 File Offset: 0x00091CE4
		public override void Update(float f)
		{
			...
			if (this.mHitTimer > 0)
			{
				...
				{
					int num = 2 + Common.Rand() % Common._M(1);
					for (int i = 0; i < num; i++)
					{
						SquidSweat squidSweat = new SquidSweat();
						this.mSweat.Add(squidSweat);
						squidSweat.mAngle = Common.DegreesToRadians((float)(Common._M(45) + Common.Rand() % Common._M1(90)));
						float num2 = Common.FloatRange(Common._M(2.5f), Common._M1(3f));
			...
			if (this.mJawCount > 0)
			{
				if (--this.mJawCount == 0)
				{
					this.mJawDelay = Common._M(400) + Common.Rand() % Common._M1(400);
				}
			}
			else if (this.mUpdateCount % this.mJawDelay == 0)
			{
				this.mJawCount = Common._M(20) + Common.Rand() % Common._M1(50);
			}
			if (this.mBlinkCel < 0 && this.mUpdateCount % this.mBlinkDelay == 0)
			{
				this.mBlinkCel = 0;
				this.mBlinkDelay = Common._M(300) + Common.Rand() % Common._M1(200);
			}
			...
			for (int num13 = 0; num13 < 4; num13++)
			{
				...
				float num14 = this.mShieldAngle + (float)num13 * 3.14159f / 2f;
				int num15 = Common._DS(Common._M(200));
				shieldQuadrantData.mSparkles.mDrawTransform.LoadIdentity();
				float num16 = GameApp.DownScaleNum(1f);
				shieldQuadrantData.mSparkles.mDrawTransform.Scale(num16, num16);
				shieldQuadrantData.mSparkles.mDrawTransform.Translate(0f, (float)(-(float)num15));
				shieldQuadrantData.mSparkles.mDrawTransform.RotateRad(num14);
				shieldQuadrantData.mSparkles.mDrawTransform.Translate(Common._S(this.mX), (float)num15 + Common._S(this.mY - (float)Common._M(100)));
		...
		// Token: 0x06000E6D RID: 3693 RVA: 0x0009457C File Offset: 0x0009277C
		...
		// Token: 0x06000E6E RID: 3694 RVA: 0x0009493C File Offset: 0x00092B3C
		...
		// Token: 0x06000E6F RID: 3695 RVA: 0x00094960 File Offset: 0x00092B60
		...
		protected int mJawDelay = 1 + Common.SafeRand() % 50;

		// Token: 0x0400174F RID: 5967
		protected int mBlinkDelay = 1 + Common.SafeRand() % 150;
