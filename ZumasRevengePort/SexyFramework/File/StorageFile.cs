...
namespace SexyFramework.File
{
	...
	public class StorageFile
	{
		...
		public static bool ReadBufferFromFile(string theFileName, Buffer theBuffer)
		{
			IsolatedStorageFile userStoreForApplication = IsolatedStorageFile.GetUserStoreForApplication();
			...
			IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, (System.IO.FileMode)3);
		...
		public static bool WriteBufferToFile(string theFileName, Buffer theBuffer)
		{
			IsolatedStorageFile userStoreForApplication = IsolatedStorageFile.GetUserStoreForApplication();
			...
			IsolatedStorageFileStream isolatedStorageFileStream = userStoreForApplication.OpenFile(theFileName, (System.IO.FileMode)2);
			if (isolatedStorageFileStream == null)
			{
				return false;
			}
		...
		public bool openRead(string fName, bool bSilent, bool bFromDocs)
		{
			...
			this.fp = this.m_userStore.OpenFile(fName, (System.IO.FileMode)3);
			if (this.fp == null)
			{
				return false;
			}
		...
		{
			this.clear();
			this.fp = this.m_userStore.OpenFile(fName, (System.IO.FileMode)2);
			if (this.fp == null)
			{
				return false;
			}
		...
		{
			return (int)(this.fp.Seek(0L, SeekOrigin.End) - 12L);
		}
