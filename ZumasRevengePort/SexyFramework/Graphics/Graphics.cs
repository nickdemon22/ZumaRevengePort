...
namespace SexyFramework.Graphics
{
	...
	public class SexyGraphics : GraphicsState
	{
		...
		protected void InitRenderInfo(SexyGraphics theSourceGraphics)
		{
		...
		protected void CalcFinalColor()
		{
			...
			{
				Color color = this.mPushedColorVector[this.mPushedColorVector.Count - 1];
				this.mFinalColor = new Color(Math.Min(255, color.mRed * this.mColor.mRed / 255), Math.Min(255, color.mGreen * this.mColor.mGreen / 255), Math.Min(255, color.mBlue * this.mColor.mBlue / 255), Math.Min(255, color.mAlpha * this.mColor.mAlpha / 255));
				return;
			}
		...
		public SexyGraphics(Image theDestImage)
		{
			this.mTransX = 0f;
			this.mTransY = 0f;
			this.mScaleX = 1f;
		...
		public SexyGraphics(SexyGraphics theGraphics)
		{
		...
		{
			Color finalColor = this.GetFinalColor();
		...
		public int WriteWordNoAutoWrapped(string theLine, int x, int y)
		{
			...
			int num = color.ToInt();
			if (((long)num & unchecked((long)-16777216)) == unchecked((long)-16777216))
			{
				num &= 16777215;
			}
