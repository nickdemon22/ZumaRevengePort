...
namespace SexyFramework.Graphics
{
	...
	public class Graphics3D
	{
		...
		public Graphics3D(SexyGraphics inGraphics, RenderDevice3D inRenderDevice, HRenderContext inRenderContext)
		{
		...
		public void DrawPrimitiveEx(uint theVertexFormat, Graphics3D.EPrimitiveType thePrimitiveType, SexyVertex2D[] theVertices, int thePrimitiveCount, Color theColor, int theDrawMode, float tx, float ty, bool blend, uint theFlags)
		{
			this.SetAsCurrentContext();
		...
		public void DrawPrimitive(uint theVertexFormat, Graphics3D.EPrimitiveType thePrimitiveType, SexyVertex2D[] theVertices, int thePrimitiveCount, Color theColor, int theDrawMode, float tx, float ty, bool blend, uint theFlags)
		{
			this.SetAsCurrentContext();
		...
		{
			this.ClearColorBuffer(Color.Black);
		}
		...
		public void SetLightingEnabled(int inLightingEnabled, bool inSetDefaultMaterialState)
		{
			...
			{
				this.SetMaterialAmbient(Color.White);
				this.SetMaterialDiffuse(Color.White, 0);
				this.SetMaterialSpecular(Color.White);
				this.SetMaterialEmissive(Color.Black);
			}
		}
		...
		protected SexyGraphics mGraphics;
		...
		public class LightColors
		{
			...
			public LightColors()
			{
				this.mDiffuse = new Color(Color.White);
				this.mSpecular = new Color(Color.Black);
				this.mAmbient = new Color(Color.Black);
				this.mAutoScale = 1f;
			}
		...
		public class PerspectiveCamera : Graphics3D.Camera
		{
			...
			public PerspectiveCamera(float inFovDegrees, float inAspectRatio, float inZNear) : this(inFovDegrees, inAspectRatio, inZNear, 10000f)
			{
			}

			// Token: 0x060005E0 RID: 1504 RVA: 0x00012A81 File Offset: 0x00010C81
			public PerspectiveCamera(float inFovDegrees, float inAspectRatio) : this(inFovDegrees, inAspectRatio, 1f, 10000f)
			{
			}
		...
		public class OffCenterPerspectiveCamera : Graphics3D.Camera
		{
			...
			public OffCenterPerspectiveCamera(float inFovDegrees, float inAspectRatio, float inOffsetX, float inOffsetY, float inZNear) : this(inFovDegrees, inAspectRatio, inOffsetX, inOffsetY, inZNear, 10000f)
			{
			}

			// Token: 0x060005EC RID: 1516 RVA: 0x00012D1F File Offset: 0x00010F1F
			public OffCenterPerspectiveCamera(float inFovDegrees, float inAspectRatio, float inOffsetX, float inOffsetY) : this(inFovDegrees, inAspectRatio, inOffsetX, inOffsetY, 1f, 10000f)
			{
			}
		...
		public class OrthoCamera : Graphics3D.Camera
		{
			...
			public OrthoCamera(float inWidth, float inHeight, float inZNear) : this(inWidth, inHeight, inZNear, 10000f)
			{
			}

			// Token: 0x060005F8 RID: 1528 RVA: 0x0001301E File Offset: 0x0001121E
			public OrthoCamera(float inWidth, float inHeight) : this(inWidth, inHeight, 1f, 10000f)
			{
			}
