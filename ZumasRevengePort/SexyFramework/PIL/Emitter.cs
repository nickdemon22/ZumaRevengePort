...
namespace SexyFramework.PIL
{
	...
	public class Emitter : MovableObject
	{
		...
		public void Deserialize(Buffer b, Dictionary<int, Deflector> deflector_ptr_map, Dictionary<int, FreeEmitter> fe_ptr_map, GlobalMembers.GetImageByIdFunc f)
		{
			...
			this.mTintColor = new Color(theColor);
		...
		public Emitter(Emitter rhs) : this()
		{
			this.CopyFrom(rhs);
		}
		...
		public void CopyFrom(Emitter rhs)
		{
			...
			{
				this.mMaskPoints = new Point[this.mNumMaskPoints];
				for (int i = 0; i < this.mNumMaskPoints; i++)
				{
					this.mMaskPoints[i] = new Point(rhs.mMaskPoints[i]);
				}
		...
		public void Draw(SexyGraphics g, float vis_mult, float tint_mult)
		{
		...
		public void Draw(SexyGraphics g, float vis_mult)
		{
		...
		public void DebugDraw(SexyGraphics g, int size)
		{
		}
