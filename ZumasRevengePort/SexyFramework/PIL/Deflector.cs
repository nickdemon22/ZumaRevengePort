...
namespace SexyFramework.PIL
{
	...
	public class Deflector
	{
		...
		public void DebugDraw(SexyGraphics g)
		{
		}
		...
		public void Apply(MovableObject p)
		{
			...
			if (Common.RotatedRectsIntersect(r, -p.mAngle, this.mUnrotatedRect, this.mBaseAngle + this.mLastSettings.mAngle))
			{
				...
				for (int i = 0; i < 4; i++)
				{
					array[i] = Common.DistFromPointToLine(this.mRotatedPoints[i], (i < 3) ? this.mRotatedPoints[i + 1] : this.mRotatedPoints[0], new Point((int)p.GetX(), (int)p.GetY()), ref num3);
					if (array[i] < num)
					{
						num = array[i];
						num2 = i;
				...
				Point point = (num2 == 3) ? this.mRotatedPoints[0] : this.mRotatedPoints[num2 + 1];
		...
		protected Point[] mRotatedPoints = new Point[]
		{
			new Point(),
			new Point(),
			new Point(),
