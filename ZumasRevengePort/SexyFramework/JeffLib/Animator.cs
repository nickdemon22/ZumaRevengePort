...
namespace JeffLib
{
	...
	public class Animator
	{
		...
		private void START_ADDITIVE(SexyGraphics g, Color val)
		{
			g.SetDrawMode(1);
			if (this.mFadeData.mFadeState == 0)
			{
		...
		private void START_ADDITIVE(SexyGraphics g)
		{
			g.SetDrawMode(0);
		...
		private void START_COLORIZED(SexyGraphics g, Color val)
		{
			g.SetColorizeImages(true);
			if (this.mFadeData.mFadeState == 0)
			{
		...
		private void STOP_COLORIZED(SexyGraphics g)
		{
			g.SetColorizeImages(false);
		}

		// Token: 0x06000782 RID: 1922 RVA: 0x00027170 File Offset: 0x00025370
		private void START_FADE(SexyGraphics g)
		{
		...
		private void STOP_FADE(SexyGraphics g)
		{
			if (!this.mDrawAdditive && !this.mDrawColorized && this.mFadeData.mFadeState != 0)
			{
				g.SetColorizeImages(false);
		...
		private void SETUP_DRAWING(SexyGraphics g)
		{
			this.START_FADE(g);
		...
		private void END_DRAWING(SexyGraphics g)
		{
			this.STOP_FADE(g);
			if (this.mDrawAdditive)
			{
		...
		public virtual void Draw(SexyGraphics g, int pX, int pY)
		{
		...
		public virtual void Draw(SexyGraphics g, float pX, float pY, bool pSmooth)
		{
			if (this.mImage == null || this.IsPaused() || this.IsDone())
			{
				return;
		...
		public virtual void DrawStretched(SexyGraphics g, float pX, float pY, float pPct)
		{
			if (this.mImage == null || this.IsPaused() || this.IsDone())
			{
				return;
		...
		public virtual void DrawStretched(SexyGraphics g, float pX, float pY, int pWidth, int pHeight)
		{
			if (this.mImage == null || this.IsPaused() || this.IsDone())
			{
				return;
		...
		public virtual void DrawRotated(SexyGraphics g, float pX, float pY, float pAngle, bool pSmooth, float pCenterX, float pCenterY)
		{
			if (this.mImage == null || this.IsPaused() || this.IsDone())
			{
				return;
