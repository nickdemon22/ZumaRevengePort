...
namespace JeffLib
{
	...
	public class BonusText
	{
		...
		public BonusText(string pText, Font pFont, float pX, float pY, float pSpeed, int pAlphaDelay)
		{
			...
			this.mSolidColor = new Color(Color.White);
			...
			this.mSpace = 5;
			this.mImageColor = new Color(Color.White);
			this.mImageCel = 0;
			this.mDone = false;
			this.mUpdateCnt = 0;
			this.mSize = 1f;
		...
		public void Draw(SexyGraphics g)
		{
			if (!this.mDone)
			{
				...
				Color color = (!this.mUseSolidColor) ? new Color((int)((GlobalMembers.gSexyAppBase.HSLToRGB(this.mHue, 255, 128) & 16777215UL) | (ulong)((ulong)((uint)(num * 255f)) << 24))) : new Color(this.mSolidColor);
				...
				if (this.mImage != null)
				{
					...
					else if (this.mImageColor != Color.White)
					{
						g.SetColorizeImages(true);
						g.SetColor(new Color(this.mImageColor)
						{
							mAlpha = (int)num
						});
					}
