...
namespace JeffLib
{
	...
	public static class JCommon
	{
		...
		{
			JCommon.StringDimensions(str, f, out widest, out height, true);
		}
		...
		{
			return JCommon.RotatePoint(pAngle, pVector, new SexyVector2(0f, 0f));
		}
		...
		{
			SexyVector2 sexyVector = JCommon.RotatePoint(pAngle, new SexyVector2(x, y), new SexyVector2(cx, cy));
			x = sexyVector.x;
			y = sexyVector.y;
		}
		...
		{
			return JCommon.UpdateToTimeStr(u, false, 2);
		}
		...
		{
			return JCommon.UpdateToTimeStr(u, use_hour_field, 2);
		}
		...
		{
			return c == JCommon.RIGHT_BUTTON || c == JCommon.DOUBLE_RIGHT_BUTTON;
		}
		...
		public static ulong MTRAND_MAX = unchecked((ulong)-1);
	}
}
