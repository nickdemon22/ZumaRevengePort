...
namespace SexyFramework.Widget
{
	...
	public class PopAnim : Widget
	{
		...
		public bool LoadSpriteDef(Buffer theBuffer, PASpriteDef theSpriteDef)
		{
		...
		public void CalcObjectPos(PASpriteInst theSpriteInst, int theObjectPosIdx, bool frozen, out PATransform theTransform, out Color theColor)
		{
			...
			if (this.mInterpolate && !frozen)
			{
				...
				if (array2[1] != null)
				{
					...
					color = new Color((int)((float)PopAnim.ClrR(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrR(array2[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrG(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrG(array2[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrB(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrB(array2[1].mColorInt) * num + 0.5f), (int)((float)PopAnim.ClrA(paobjectPos.mColorInt) * (1f - num) + (float)PopAnim.ClrA(array2[1].mColorInt) * num + 0.5f));
				}
				else
				{
					patransform.CopyFrom(paobjectPos.mTransform);
					color = new Color(PopAnim.ClrR(paobjectPos.mColorInt), PopAnim.ClrG(paobjectPos.mColorInt), PopAnim.ClrB(paobjectPos.mColorInt), PopAnim.ClrA(paobjectPos.mColorInt));
				}
			}
			else
			{
		...
		public void SaveStateSpriteInst(ref Buffer theBuffer, PASpriteInst theSpriteInst)
		{
		...
		public void LoadStateSpriteInst(Buffer theBuffer, PASpriteInst theSpriteInst)
		{
		...
		public void DrawParticleEffects(SexyGraphics g, PASpriteInst theSpriteInst, PATransform theTransform, Color theColor, bool front)
		{
			if (theSpriteInst.mParticleEffectVector.Count > 0)
			{
				for (int i = 0; i < theSpriteInst.mParticleEffectVector.Count; i++)
		...
		public virtual void DrawSprite(SexyGraphics g, PASpriteInst theSpriteInst, PATransform theTransform, Color theColor, bool additive, bool parentFrozen)
		{
			...
			for (int i = 0; i < paframe.mFrameObjectPosVector.Count; i++)
			{
				...
				Color color = new Color(mCurColor.mRed * theColor.mRed * paobjectInst.mColorMult.mRed / 65025, mCurColor.mGreen * theColor.mGreen * paobjectInst.mColorMult.mGreen / 65025, mCurColor.mBlue * theColor.mBlue * paobjectInst.mColorMult.mBlue / 65025, mCurColor.mAlpha * theColor.mAlpha * paobjectInst.mColorMult.mAlpha / 65025);
				if (color.mAlpha != 0)
				{
					if (paobjectPos.mIsSprite)
					{
		...
		public virtual void DrawSpriteMirrored(SexyGraphics g, PASpriteInst theSpriteInst, PATransform theTransform, Color theColor, bool additive, bool parentFrozen)
		{
			this.DrawParticleEffects(g, theSpriteInst, theTransform, new Color(theColor), false);
			PAFrame paframe = theSpriteInst.mDef.mFrames[(int)theSpriteInst.mFrameNum];
			PATransform patransform = null;
			bool flag = parentFrozen || theSpriteInst.mDelayFrames > 0 || paframe.mHasStop;
			for (int i = 0; i < paframe.mFrameObjectPosVector.Count; i++)
			{
				...
				Color color = new Color(mCurColor.mRed * theColor.mRed / 255, mCurColor.mGreen * theColor.mGreen / 255, mCurColor.mBlue * theColor.mBlue / 255, mCurColor.mAlpha * theColor.mAlpha / 255);
				if (color.mAlpha != 0)
				{
					if (paobjectPos.mIsSprite)
					{
						this.DrawSpriteMirrored(g, theSpriteInst.mChildren[paobjectPos.mObjectNum].mSpriteInst, patransform2, new Color(color), paobjectPos.mIsAdditive || additive, flag);
					}
					else
					{
						PAImage paimage = this.mImageVector[(int)paobjectPos.mResNum];
		...
		public virtual bool Load_LoadPam(string theFileName)
		{
			...
			uint num = unchecked((uint)buffer.ReadInt32());
			if (num != PopAnim.PAM_MAGIC_UINT)
			{
		...
		public void CopyForm(PopAnim rhs)
		{
			...
			this.mColor = new Color(rhs.mColor);
		...
		public bool SaveState(ref Buffer theBuffer)
		{
		...
		public bool BlendTo(string theFrameLabel, int theBlendTicks, int theAnimStartDelay)
		{
			...
			if (!this.mTransDirty)
			{
				this.UpdateTransforms(this.mMainSpriteInst, null, new Color(this.mColor), false);
		...
		public static ulong PAM_MAGIC = unchecked((ulong)-1158669996);

		public const uint PAM_MAGIC_UINT = 0xBAF01954U;
		...
		public Buffer mCRCBuffer = new Buffer();
