...
namespace SexyFramework.Widget
{
	...
	public class ScrollWidget : Widget, ProxyWidgetListener, IDisposable
	{
		...
		public override void TouchMoved(SexyAppBase.Touch touch)
		{
			...
			{
				Point point = this.GetAbsPos() - this.mClientLastDown.GetAbsPos();
				Point impliedObject = new Point(touch.location.mX, touch.location.mY);
				Point point2 = impliedObject + point;
				Point thePoint = new Point(point2.mX + this.mClientLastDown.mX, point2.mY + this.mClientLastDown.mY);
		...
		private static void DrawHorizontalStretchableImage(SexyGraphics g, Image image, Rect destRect)
		{
		...
		private static void DrawVerticalStretchableImage(SexyGraphics g, Image image, Rect destRect)
		{
			int width = image.GetWidth();
			int height = image.GetHeight();
		...
		public void DrawProxyWidget(SexyGraphics g, ProxyWidget proxyWidget)
		{
		...
		protected Widget GetClientWidgetAt(SexyAppBase.Touch touch)
		{
			...
			{
				Point absPos = this.mClient.GetAbsPos();
				Point absPos2 = this.mClientLastDown.GetAbsPos();
