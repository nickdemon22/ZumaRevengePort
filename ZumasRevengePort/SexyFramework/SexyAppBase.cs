...
namespace SexyFramework
{
	...
	public class SexyAppBase : ButtonListener, DialogListener
	{
		...
		public virtual void Dispose()
		{
			...
			{
				SexyFramework.Widget.Widget value = keyValuePair.Value;
		...
		public virtual bool WriteBufferToFile(string theFileName, Buffer theBuffer)
		{
		...
		public bool ReadBufferFromStream(string theFileName, ref Buffer theBuffer)
		{
			theBuffer.Clear();
			string normalized = theFileName.Replace('/', Path.DirectorySeparatorChar).Replace('\\', Path.DirectorySeparatorChar);
			string contentPath = Path.Combine(AppContext.BaseDirectory, "Content", normalized);
			if (System.IO.File.Exists(contentPath))
			{
				byte[] data = System.IO.File.ReadAllBytes(contentPath);
				if (data.Length > 0)
				{
					theBuffer.SetData(data, data.Length);
					return true;
				}
			}
			try
			{
				string titlePath = "Content/" + normalized.Replace(Path.DirectorySeparatorChar, '/');
				using (Stream stream = TitleContainer.OpenStream(titlePath))
				{
					...
					{
						int read = stream.Read(array, totalRead, array.Length - totalRead);
						if (read <= 0)
						{
							break;
						}
					...
					if (totalRead > 0)
					{
						theBuffer.SetData(array, totalRead);
						return true;
					}
				}
			}
		...
		public bool ReadBufferFromFile(string theFileName, ref Buffer theBuffer)
		{
		...
		public virtual bool CheckSignature(Buffer theBuffer, string theFileName)
		{
		...
		public virtual void ColorizeImage(SharedImageRef theImage, Color theColor)
		{
		}

		// Token: 0x06000F28 RID: 3880 RVA: 0x0004B403 File Offset: 0x00049603
		public virtual DeviceImage CreateColorizedImage(Image theImage, Color theColor)
		{
			return null;
		}
			...
			if (this.mCleanupSharedImages)
			{
				foreach (KeyValuePair<KeyValuePair<string, string>, SharedImage> keyValuePair in this.mSharedImageMap)
				{
					...
					if (value.mImage != null && value.mRefCount == 0)
					{
						...
						value.mImage = null;
						this.mRemoveList.Add(keyValuePair.Key);
					}
				}
		...
		public ulong HSLToRGB(int h, int s, int l)
		{
			...
			return unchecked((ulong)-16777216) | (ulong)((ulong)((long)num6) << 16) | (ulong)((ulong)((long)num7) << 8) | (ulong)((long)num8);
		}
		...
		public ulong RGBToHSL(int r, int g, int b)
		{
			...
			return unchecked((ulong)-16777216) | (ulong)((long)num3) | (ulong)((long)((long)num4 << 8)) | (ulong)((long)((long)num5 << 16));
		}
		...
		public void HSLToRGB(ulong[] theSource, ulong[] theDest, int theSize)
		{
			for (int i = 0; i < theSize; i++)
			{
				...
				theDest[i] = ((num & unchecked((ulong)-16777216)) | (this.HSLToRGB((int)(num & 255UL), (int)(num >> 8) & 255, (int)(num >> 16) & 255) & 16777215UL));
			}
		}
		...
		public void RGBToHSL(ulong[] theSource, ulong[] theDest, int theSize)
		{
			...
			{
				ulong num = theSource[i];
				theDest[i] = ((num & unchecked((ulong)-16777216)) | (this.RGBToHSL((int)(num >> 16 & 255UL), (int)(num >> 8) & 255, (int)(num & 255UL)) & 16777215UL));
			}
		}
		...
		public class Touch
		{
			...
			public void SetTouchInfo(Point loc, _TouchPhase _phase, double _timestamp)
			{
