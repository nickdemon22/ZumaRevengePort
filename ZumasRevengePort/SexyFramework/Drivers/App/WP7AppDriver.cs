...
namespace SexyFramework.Drivers.App
{
	...
	public class WP7AppDriver : IAppDriver
	{
		...
		public override void Init()
		{
			...
			this.IsScreenSaver();
			this.mApp.mScreenBounds.mX = 0;
			this.mApp.mScreenBounds.mY = 0;
			this.mApp.mScreenBounds.mWidth = this.mApp.mWidth;
			this.mApp.mScreenBounds.mHeight = this.mApp.mHeight;
			BaseXNARenderDevice renderDevice = this.mXNAGraphicsDriver.mXNARenderDevice;
			renderDevice.ApplyLetterboxViewport(this.mApp.mWidth, this.mApp.mHeight);
			Viewport viewport = renderDevice.mDevice.GraphicsDevice.Viewport;
			Rect mouseDest = new Rect(0, 0, this.mApp.mWidth, this.mApp.mHeight);
			Rect mouseSource = new Rect(viewport.X, viewport.Y, viewport.Width, viewport.Height);
			this.mApp.mWidgetManager.Resize(mouseDest, mouseSource);
		...
		public override void Popup(string theString)
		{
			...
			Debug.WriteLine("Game popup: " + theString);
			if (OperatingSystem.IsWindows())
			{
				MessageBoxW(IntPtr.Zero, theString, "Zuma's Revenge!", 0U);
			}
		}
		...
		{
			if (this.mXNAGraphicsDriver == null || this.mXNAGraphicsDriver.mXNARenderDevice == null)
			{
				return;
			}
			BaseXNARenderDevice renderDevice = this.mXNAGraphicsDriver.mXNARenderDevice;
			int width = renderDevice.mDevice.PreferredBackBufferWidth;
			int height = renderDevice.mDevice.PreferredBackBufferHeight;
			if (this.mApp.mPreferredWidth > 0 && this.mApp.mPreferredHeight > 0)
			{
				width = this.mApp.mPreferredWidth;
				height = this.mApp.mPreferredHeight;
			}
			renderDevice.SetBackBufferSize(width, height, !wantWindowed);
			renderDevice.EnsureDeviceChanges();
			renderDevice.ApplyLetterboxViewport(this.mApp.mWidth, this.mApp.mHeight);
			this.mApp.mIsWindowed = wantWindowed;
			this.mApp.mIsPhysWindowed = wantWindowed;
			Viewport viewport = renderDevice.mDevice.GraphicsDevice.Viewport;
			Rect mouseDest = new Rect(0, 0, this.mApp.mWidth, this.mApp.mHeight);
			Rect mouseSource = new Rect(viewport.X, viewport.Y, viewport.Width, viewport.Height);
			this.mApp.mWidgetManager.Resize(mouseDest, mouseSource);
		...
		public override bool CheckSignature(Buffer theBuffer, string theFileName)
		{
			return true;
		}
		...
		public override bool WriteBufferToFile(string theFileName, Buffer theBuffer)
		{
			return true;
		}

		// Token: 0x060000F8 RID: 248 RVA: 0x000048AA File Offset: 0x00002AAA
		public override bool ReadBufferFromFile(string theFileName, Buffer theBuffer, bool dontWriteToDemo)
		{
			return true;
		}
		...
		public override void Draw()
		{
			...
			renderDevice.EnsureDeviceChanges();
			renderDevice.ApplyLetterboxViewport(this.mApp.mWidth, this.mApp.mHeight);
			this.mXNAGraphicsDriver.ClearColorBuffer(Color.Black);
			this.DrawDirtyStuff();
		}
		...
		public string GetProductVersion(string thePath)
		{
			...
			return "v" + fullName.Split(new char[]
			{
				'='
			})[1].Split(new char[]
			{
