...
namespace SexyFramework.Drivers.Graphics
{
	...
	public class BaseXNARenderDevice : RenderDevice3D
	{
		...
		public BaseXNARenderDevice(IGraphicsDriver theDriver)
		{
			...
			this.mDevice.IsFullScreen = false; //avernus
		...
		public BaseXNARenderDevice(Game game)
		{
			...
			this.mBatchedTriangleBuffer = new VertexPositionColorTexture[BaseXNARenderDevice.mBatchedTriangleSize];
			this.mCurrentContex = null;
			this.mDevice.IsFullScreen = false; //orbit
		}
		...
		public override void SetCurrentContext(HRenderContext theContext)
		{
			...
			if (theContext == null || theContext.GetPointer() == null)
			{
				...
				this.mStateMgr.SetProjectionTransform(Matrix.CreateOrthographicOffCenter(0f, (float)this.mWidth, (float)this.mHeight, 0f, -1000f, 1000f));
				this.SetViewport(0, 0, this.mWidth, this.mHeight, 0f, 1f);
		...
		public override void DrawPrimitiveEx(uint theVertexFormat, Graphics3D.EPrimitiveType thePrimitiveType, SexyVertex2D[] theVertices, int thePrimitiveCount, Color theColor, int theDrawMode, float tx, float ty, bool blend, uint theFlags)
		{
			int num = 0;
			...
			this.mStateMgr.PushState();
			Color color;
			color = new Color(theColor.mRed, theColor.mGreen, theColor.mBlue, theColor.mAlpha);
			...
			if ((theVertexFormat & 4U) != 0U && (color.PackedValue != 0U || tx != 0f || ty != 0f || this.mTransformStack.Count != 0))
			{
				for (int i = 0; i < num; i++)
				{
					...
					if (theVertices[i].color == Color.Zero)
					{
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (theVertices[j].color == Color.Zero)
				{
					array[j].Color = color;
				}
				else
		...
		public override void ClearColorBuffer(Color inColor)
		{
		...
		public override void SetMaterialAmbient(Color inColor, int inVertexColorComponent)
		{
		}

		// Token: 0x060002AC RID: 684 RVA: 0x000088B3 File Offset: 0x00006AB3
		public override void SetMaterialDiffuse(Color inColor, int inVertexColorComponent)
		{
		}

		// Token: 0x060002AD RID: 685 RVA: 0x000088B5 File Offset: 0x00006AB5
		public override void SetMaterialSpecular(Color inColor, int inVertexColorComponent, float inPower)
		{
		}

		// Token: 0x060002AE RID: 686 RVA: 0x000088B7 File Offset: 0x00006AB7
		public override void SetMaterialEmissive(Color inColor, int inVertexColorComponent)
		{
		}
		...
		public override void FillRect(Rect theRect, Color theColor, int theDrawMode)
		{
			if (!this.PreDraw())
			{
				return;
		...
		public override void FillScanLinesWithCoverage(RenderDevice.Span theSpans, int theSpanCount, Color theColor, int theDrawMode, string theCoverage, int theCoverX, int theCoverY, int theCoverWidth, int theCoverHeight)
		{
		}

		// Token: 0x060002C6 RID: 710 RVA: 0x00008DF4 File Offset: 0x00006FF4
		public override void FillPoly(Point[] theVertices, int theNumVertices, Rect theClipRect, Color theColor, int theDrawMode, int tx, int ty)
		{
			if (theNumVertices < 3)
			{
				return;
		...
		public override void DrawLine(double theStartX, double theStartY, double theEndX, double theEndY, Color theColor, int theDrawMode, bool antiAlias)
		{
			if (!this.PreDraw())
			{
				return;
		...
		public override void Blt(Image theImage, int theX, int theY, Rect theSrcRect, Color theColor, int theDrawMode)
		{
		...
		public override void BltF(Image theImage, float theX, float theY, Rect theSrcRect, Rect theClipRect, Color theColor, int theDrawMode)
		{
		...
		public override void BltRotated(Image theImage, float theX, float theY, Rect theSrcRect, Rect theClipRect, Color theColor, int theDrawMode, double theRot, float theRotCenterX, float theRotCenterY)
		{
		...
		private void BltTransformed(Image theImage, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, SexyMatrix3 theTransform, bool linearFilter, float theX, float theY, bool center)
		{
			if (!this.PreDraw())
			{
				return;
		...
		public override void BltMatrix(Image theImage, float x, float y, SexyMatrix3 theMatrix, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, bool blend)
		{
		...
		public override void BltTriangles(Image theImage, SexyVertex2D[,] theVertices, int theNumTriangles, Color theColor, int theDrawMode, float tx, float ty, bool blend, Rect theClipRect)
		{
		...
		public void BltTrianglesHelper(Image theImage, SexyVertex2D[,] theVertices, int theNumTriangles, Color theColor, int theDrawMode, float tx, float ty, bool blend, Rect theClipRect)
		{
			...
			if ((double)xnatextureData.mMaxTotalU <= 1.0 && (double)xnatextureData.mMaxTotalV <= 1.0)
			{
				...
				if (flag)
				{
					...
					for (int i = 0; i < theNumTriangles; i++)
					{
						...
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[0].x, array[0].y, num), (theVertices[i, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[i, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[i, 0].u * xnatextureData.mMaxTotalU, theVertices[i, 0].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[1].x, array[1].y, num), (theVertices[i, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[i, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[i, 1].u * xnatextureData.mMaxTotalU, theVertices[i, 1].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(array[2].x, array[2].y, num), (theVertices[i, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[i, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[i, 2].u * xnatextureData.mMaxTotalU, theVertices[i, 2].v * xnatextureData.mMaxTotalV));
				...
				if (!BaseXNARenderDevice.SUPPORT_HW_CLIP && flag2)
				{
					for (int k = 0; k < theNumTriangles; k++)
					{
						...
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[k, 0].x, theVertices[k, 0].y, num), (theVertices[k, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 0].u * xnatextureData.mMaxTotalU, theVertices[k, 0].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[k, 1].x, theVertices[k, 1].y, num), (theVertices[k, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 1].u * xnatextureData.mMaxTotalU, theVertices[k, 1].v * xnatextureData.mMaxTotalV));
						this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[k, 2].x, theVertices[k, 2].y, num), (theVertices[k, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[k, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[k, 2].u * xnatextureData.mMaxTotalU, theVertices[k, 2].v * xnatextureData.mMaxTotalV));
						this.AdjustVertsForAtlas(0, ref this.mBatchedTriangleBuffer, this.mBatchedTriangleIndex - 3, 3, 0U, 32, 0);
						if (!BaseXNARenderDevice.SUPPORT_HW_CLIP && flag2)
						{
				...
				else
				{
					...
					while (m < theNumTriangles)
					{
						...
						while (n < num2)
						{
							this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 0].x, theVertices[m, 0].y, num), (theVertices[m, 0].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 0].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 0].u * xnatextureData.mMaxTotalU, theVertices[m, 0].v * xnatextureData.mMaxTotalV));
							this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 1].x, theVertices[m, 1].y, num), (theVertices[m, 1].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 1].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 1].u * xnatextureData.mMaxTotalU, theVertices[m, 1].v * xnatextureData.mMaxTotalV));
							this.mBatchedTriangleBuffer[this.mBatchedTriangleIndex++] = new VertexPositionColorTexture(new Vector3(theVertices[m, 2].x, theVertices[m, 2].y, num), (theVertices[m, 2].color != Color.Zero) ? this.GetXNAColor(theVertices[m, 2].color) : this.GetXNAColor(theColor), new Vector2(theVertices[m, 2].u * xnatextureData.mMaxTotalU, theVertices[m, 2].v * xnatextureData.mMaxTotalV));
							n += 3;
							m++;
						}
		...
		public override void BltMirror(Image theImage, int theX, int theY, Rect theSrcRect, Color theColor, int theDrawMode)
		{
			this.mTransform.Reset();
		...
		public override void BltStretched(Image theImage, Rect theDestRect, Rect theSrcRect, Rect theClipRect, Color theColor, int theDrawMode, bool fastStretch, bool mirror)
		{
		...
		public override void SetGlobalAmbient(Color inColor)
		{
		}
		...
		public void Init(int width, int height)
		{
			...
			this.mAdditiveState.AlphaDestinationBlend = (Microsoft.Xna.Framework.Graphics.Blend)0;
			this.mAdditiveState.ColorDestinationBlend = (Microsoft.Xna.Framework.Graphics.Blend)0;
			this.mAdditiveState.AlphaSourceBlend = (Microsoft.Xna.Framework.Graphics.Blend)4;
			this.mAdditiveState.ColorSourceBlend = (Microsoft.Xna.Framework.Graphics.Blend)4;
			this.mNormalState.AlphaDestinationBlend = (Microsoft.Xna.Framework.Graphics.Blend)5;
			this.mNormalState.ColorDestinationBlend = (Microsoft.Xna.Framework.Graphics.Blend)5;
			this.mNormalState.AlphaSourceBlend = (Microsoft.Xna.Framework.Graphics.Blend)4;
			this.mNormalState.ColorSourceBlend = (Microsoft.Xna.Framework.Graphics.Blend)4;
		...
		private void BltClipF(Image theImage, float theX, float theY, Rect theSrcRect, Rect theClipRect, Color theColor, int theDrawMode)
		{
		...
		public void BltNoClipF(Image theImage, float theX, float theY, Rect theSrcRect, Color theColor, int theDrawMode, bool linearFilter)
		{
			if (this.mTransformStack.Count != 0)
			{
		...
		public void BltTransformHelper(Image theImage, Rect theClipRect, Color theColor, int theDrawMode, Rect theSrcRect, SexyTransform2D theTransform, bool linearFilter, float theX, float theY, bool center)
		{
		...
		public override void DrawSprite(Image theImage, Color theColor, int theDrawMode, SexyTransform2D theTransform, Rect theSrcRect, bool center)
		{
			Image image = theImage;
			image.InitAtalasState();
			theImage = this.SetupAtlasState(0, theImage);
			...
			this.SetupDrawMode(theDrawMode);
			XNATextureData xnatextureData = (XNATextureData)memoryImage.GetRenderData();
			new Color(theColor.mRed, theColor.mGreen, theColor.mBlue, theColor.mAlpha);
			Rectangle rectangle;
			rectangle = new Rectangle(image.mAtlasStartX + theSrcRect.mX, image.mAtlasStartY + theSrcRect.mY, theSrcRect.mWidth, theSrcRect.mHeight);
			Texture2D texture2D = xnatextureData.mTextures[0].mTexture;
			Vector2 vector;
			vector = new Vector2(theTransform.mMatrix.M41, theTransform.mMatrix.M42);
			Vector2 vector2;
			if (center)
			{
		...
		public void BltHelper(Image theImage, float theX, float theY, Rect theSrcRect, Color theColor, int theDrawMode, bool linearFilter)
		{
			Image image = theImage;
			image.InitAtalasState();
			theImage = this.SetupAtlasState(0, theImage);
		...
		public SurfaceFormat GetXnaFormat(PixelFormat theFormat)
		{
			...
			case PixelFormat.PixelFormat_A8R8G8B8:
				return (Microsoft.Xna.Framework.Graphics.SurfaceFormat)0;
			case PixelFormat.PixelFormat_A4R4G4B4:
				return (Microsoft.Xna.Framework.Graphics.SurfaceFormat)3;
			case (PixelFormat)3:
				break;
			case PixelFormat.PixelFormat_R5G6B5:
				return (Microsoft.Xna.Framework.Graphics.SurfaceFormat)1;
			default:
		...
		public CompareFunction GetXNACompareFunc(Graphics3D.ECompareFunc func)
		{
			...
			case Graphics3D.ECompareFunc.COMPARE_NEVER:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)1;
			case Graphics3D.ECompareFunc.COMPARE_LESS:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)2;
			case Graphics3D.ECompareFunc.COMPARE_EQUAL:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)4;
			case Graphics3D.ECompareFunc.COMPARE_LESSEQUAL:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)3;
			case Graphics3D.ECompareFunc.COMPARE_GREATER:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)6;
			case Graphics3D.ECompareFunc.COMPARE_NOTEQUAL:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)7;
			case Graphics3D.ECompareFunc.COMPARE_GREATEREQUAL:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)5;
			case Graphics3D.ECompareFunc.COMPARE_ALWAYS:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)0;
			default:
				return (Microsoft.Xna.Framework.Graphics.CompareFunction)1;
			}
		}
		...
		public Blend GetXNABlendMode(Graphics3D.EBlendMode mode)
		{
			...
			case Graphics3D.EBlendMode.BLEND_ZERO:
				return (Microsoft.Xna.Framework.Graphics.Blend)1;
			case Graphics3D.EBlendMode.BLEND_ONE:
				return (Microsoft.Xna.Framework.Graphics.Blend)0;
			case Graphics3D.EBlendMode.BLEND_SRCCOLOR:
				return (Microsoft.Xna.Framework.Graphics.Blend)2;
			case Graphics3D.EBlendMode.BLEND_INVSRCCOLOR:
				return (Microsoft.Xna.Framework.Graphics.Blend)3;
			case Graphics3D.EBlendMode.BLEND_SRCALPHA:
				return (Microsoft.Xna.Framework.Graphics.Blend)4;
			case Graphics3D.EBlendMode.BLEND_INVSRCALPHA:
				return (Microsoft.Xna.Framework.Graphics.Blend)5;
			case (Graphics3D.EBlendMode)7:
			case (Graphics3D.EBlendMode)8:
				break;
			case Graphics3D.EBlendMode.BLEND_DESTCOLOR:
				return (Microsoft.Xna.Framework.Graphics.Blend)6;
			case Graphics3D.EBlendMode.BLEND_INVDESTCOLOR:
				return (Microsoft.Xna.Framework.Graphics.Blend)7;
			case Graphics3D.EBlendMode.BLEND_SRCALPHASAT:
				return (Microsoft.Xna.Framework.Graphics.Blend)12;
			default:
		...
		{
			Matrix result;
			result = new Matrix(mat.m00, mat.m01, mat.m02, mat.m03, mat.m10, mat.m11, mat.m12, mat.m13, mat.m20, mat.m21, mat.m22, mat.m23, mat.m30, mat.m31, mat.m32, mat.m33);
			return result;
		}
		...
		{
			Matrix result;
			result = new Matrix(mat.m00, mat.m10, mat.m20, 0f, mat.m01, mat.m11, mat.m21, 0f, 0f, 0f, mat.m22, 0f, mat.m02, mat.m12, 0f, 1f);
			return result;
		}

		// Token: 0x060002F3 RID: 755 RVA: 0x0000B802 File Offset: 0x00009A02
		public XnaColor GetXNAColor(Color color)
		{
			return color;
		}
		...
		private static bool TryMapPrimitiveType(int inPrimType, out PrimitiveType primitiveType)
		{
			switch (inPrimType)
			{
			case 2:
		...
		{
			if (inPrimCount <= 0 || inVertData == null || inVertData.Length == 0)
			{
				return;
			}
			if (!BaseXNARenderDevice.TryMapPrimitiveType(inPrimType, out PrimitiveType primitiveType))
			{
				return;
			}
			if (inDoCommit)
		...
		public void EnsureDeviceChanges()
		{
			if (!this.mPendingDeviceApply)
			{
				return;
			}
			this.mDevice.ApplyChanges();
			this.mPendingDeviceApply = false;
		}

		public void SetBackBufferSize(int width, int height, bool fullscreen)
		{
			this.mDevice.PreferredBackBufferWidth = width;
			this.mDevice.PreferredBackBufferHeight = height;
			this.mDevice.IsFullScreen = fullscreen;
			this.mScreenWidth = width;
			this.mScreenHeight = height;
			this.mRenderRect = new Rectangle(0, 0, width, height);
			this.mPendingDeviceApply = true;
		}

		public void ApplyLetterboxViewport(int gameWidth, int gameHeight)
		{
			int backW = this.mScreenWidth;
			int backH = this.mScreenHeight;
			if (backW <= 0 || backH <= 0)
			{
				backW = this.mDevice.PreferredBackBufferWidth;
				backH = this.mDevice.PreferredBackBufferHeight;
			}
			float gameAspect = (float)gameWidth / gameHeight;
			float backAspect = (float)backW / backH;
			int vpX;
			int vpY;
			int vpW;
			int vpH;
			if (backAspect > gameAspect)
			{
				vpH = backH;
				vpW = Math.Max(1, (int)(backH * gameAspect));
				vpX = (backW - vpW) / 2;
				vpY = 0;
			}
			else
			{
				vpW = backW;
				vpH = Math.Max(1, (int)(backW / gameAspect));
				vpX = 0;
				vpY = (backH - vpH) / 2;
			}
			this.SetViewport(vpX, vpY, vpW, vpH, 0f, 1f);
			this.mStateMgr.SetProjectionTransform(Matrix.CreateOrthographicOffCenter(0f, (float)gameWidth, (float)gameHeight, 0f, -1000f, 1000f));
		}

		public void SetOrientation(int Orientation)
		{
			this.mPendingDeviceApply = true;
			this.EnsureDeviceChanges();
		}
		...
		public Rectangle mRenderRect = new Rectangle(0, 0, 1066, 640);

		private bool mPendingDeviceApply;
