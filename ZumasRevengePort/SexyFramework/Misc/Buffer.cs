...
namespace SexyFramework.Misc
{
	...
	public class Buffer
	{
		...
		public string UTF8ToWideString()
		{
			...
			{
				string text2 = "";
				int nextUTF8CharFromStream = Buffer.GetNextUTF8CharFromStream(dataPtr, num, i, ref text2);
		...
		public void WriteByte(byte theByte)
		{
			...
			else
			{
				...
				List<byte> list;
				int num2;
				(list = this.mData)[num2 = this.mWriteBitPos / 8] = (byte)(list[num2] | (theByte << num));
		...
		public void WriteNumBits(int theNum, int theBits)
		{
			for (int i = 0; i < theBits; i++)
			{
				...
				if ((theNum & 1 << i) != 0)
				{
					List<byte> list;
					int num;
					(list = this.mData)[num = this.mWriteBitPos / 8] = (byte)(list[num] | (1 << (this.mWriteBitPos % 8)));
				}
				this.mWriteBitPos++;
			}
			if (this.mWriteBitPos > this.mDataBitSize)
		...
		public string ReadUTF8String()
		{
			...
			{
				string text2 = "";
				int nextUTF8CharFromStream = Buffer.GetNextUTF8CharFromStream(theBuffer, num2, num3, ref text2);
				if (nextUTF8CharFromStream == 0)
				{
					break;
				}
		...
		{
			return Buffer.GetNextUTF8CharFromStream(theBuffer, 0, theLen, ref theChar);
		}
		...
		private static int GetNextUTF8CharFromStream(byte[] theBuffer, int start, int theLen, ref string theChar)
		{
			...
			if ((num3 & 128) != 0)
			{
				...
				array[num4++] = num3;
				int num5 = 0;
				while (num5 < Buffer.aMaskData.Length && (num3 & (int)Buffer.aMaskData[num5]) != ((int)Buffer.aMaskData[num5] << 1 & (int)Buffer.aMaskData[num5]))
				{
					num5++;
				}
				if (num5 >= Buffer.aMaskData.Length)
				{
					return 0;
				}
				num3 &= (int)(~(int)Buffer.aMaskData[num5]);
				int num6 = num5 + 1;
				if (num6 < 2 || num6 > 6)
				{
					return 0;
		...
		private static ushort[] aMaskData = new ushort[]
		{
			192,
			224,
			240,
